...
@export var bridgette_sprite : Sprite2D
@export var full_simon_sprite : Sprite2D
@export var full_dawn_sprite : Sprite2D

@export var osimon_simon_sprite : Sprite2D
@export var osimon_dawn_sprite : Sprite2D

@export var odawn_simon_sprite: Sprite2D
@export var odawn_dawn_sprite : Sprite2D
...
func make_choice():
	...
	$SimonButton.mouse_filter = Control.MOUSE_FILTER_STOP
	$SimonButton.disabled = false
	$DawnButton.disabled = false 
	$DawnButton.mouse_filter = Control.MOUSE_FILTER_STOP
	$SimonButton.focus_mode = Control.FOCUS_ALL
	$DawnButton.focus_mode = Control.FOCUS_ALL
	$SimonButton.grab_focus()

# button effects of those choices made
func _on_simon_button_pressed() -> void:
	$SimonButton.disabled = true
	$DawnButton.disabled = true
	$DawnButton.mouse_filter = Control.MOUSE_FILTER_IGNORE
	$SimonButton.mouse_filter = Control.MOUSE_FILTER_IGNORE
	#ignore focus
	$SimonButton.focus_mode = Control.FOCUS_NONE
	$DawnButton.focus_mode = Control.FOCUS_NONE
	print("SimonTime")
...
func start_zoom():
	...
	SoundManager.fade_in_bgs("Restaurant", 2.0,0,-20)
...
func simon_talking_speed():
	# slow bridgette
	bridgette_sprite.use_parent_material = true
	...
	full_dawn_sprite.use_parent_material = true
	full_dawn_sprite.self_modulate.a = 0.9
	
	osimon_dawn_sprite.use_parent_material = true
	osimon_dawn_sprite.self_modulate.a = 0.9
	
	odawn_dawn_sprite.use_parent_material = true
	odawn_dawn_sprite.self_modulate.a = 0.9
	
	# speed up simon
	full_simon_sprite.use_parent_material = false
	full_simon_sprite.modulate.a = 1
	
	osimon_simon_sprite.use_parent_material = false
	osimon_simon_sprite.self_modulate.a = 1
	
	ocoffee_simon.speed_scale = 2
	ocoffee_simon.self_modulate.a = 1
	
	odawn_simon_sprite.use_parent_material = false
	odawn_simon_sprite.self_modulate.a = 1

func bridgette_talking_speed():
	# speed up bridgette
	bridgette_sprite.use_parent_material = false
...
func dawn_talking_speed():
	...
	full_dawn_sprite.use_parent_material = false
	full_dawn_sprite.self_modulate.a = 1
	
	osimon_dawn_sprite.use_parent_material = false
	osimon_dawn_sprite.self_modulate.a = 1
	
	odawn_dawn_sprite.use_parent_material = false
...
func turn_mirror_off():
	$MirrorLayer.use_parent_material = true
	fade_tween_out($MirrorLayer)

func intro_music():
	SoundManager.play_mfx("Intro")
	await get_tree().create_timer(25).timeout
	SoundManager.fade_out("Intro",4.0)


func _on_dawn_button_mouse_entered() -> void:
	SoundManager.play_sfx("Dawn")


func _on_simon_button_mouse_entered() -> void:
